...
const getTenants = async (req, res) => {
  try {
    ...
    if (search) {
      conds.push(`(pt.name ILIKE $${idx} OR pt.phone ILIKE $${idx})`);
      params.push(`%${search}%`);
      idx++;
    }
...
const createTenant = async (req, res) => {
  ...
  try {
    ...
    const status = room_id ? "active" : "pending";
    ...
    await client.query(
      `INSERT INTO activity_logs (pg_id,action,entity_type,entity_id,performed_by_name,performed_by_role) VALUES ($1,$2,'tenant',$3,$4,$5)`,
      [
        pgId,
        `New tenant added: ${name}`,
        tenant.rows[0].id,
  ...
  try {
    ...
    if (old.rows[0]?.room_id) {
      const stillOccupied = await client.query(
        "SELECT COUNT(*) FROM pg_tenants WHERE room_id=$1 AND status=$2 AND id!=$3",
        ["active", old.rows[0].room_id, tenantId],
      );
