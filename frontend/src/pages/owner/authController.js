...
const login = async (req, res) => {
  try {
    ...
    if (adminRes.rows.length) {
      ...
      return res.json({
        token,
        user: { id: admin.id, name: admin.name, email: admin.email, role: 'master_admin' },
      });
    }
    ...
    if (ownerRes.rows.length) {
      ...
      if (!owner.password)
        return res.status(400).json({ message: 'Account setup not complete. Please use your invite link.' });
...
const inviteOwner = async (req, res) => {
  try {
    ...
    if (existing.rows.length && existing.rows[0].invite_accepted) {
      return res.status(400).json({ message: 'Owner already registered with this email/phone' });
    }
  ...
  try {
    ...
    if (usedInvite.rows.length && usedInvite.rows[0].is_used) {
      return res.status(400).json({
        message: 'This invite link has already been used. Please log in instead.',
        code: 'ALREADY_USED',
      });
    }
...
const registerOwner = async (req, res) => {
  ...
  try {
    ...
    await client.query(
      "UPDATE invites SET is_used=TRUE WHERE token=$1",
      [token]
    );
...
module.exports = { login, getMe, inviteOwner, registerOwner, validateInvite, inviteTenant };
