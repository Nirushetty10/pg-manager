...
// Current month as DATE string 'YYYY-MM-DD' (first of month)
const currentMonthDate = (() => {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-01`;
})();

// Month options for dropdown — current + past 11
const MONTH_OPTIONS = (() => {
  const opts = [];
  const now = new Date();
  for (let i = 0; i < 12; i++) {
...
function OutstandingTable({ ledger, onRecord, canCreate }) {
  ...
  const [sortBy, setSortBy] = useState("amount"); // 'amount' | 'oldest'

  const filtered = useMemo(() => {
    let rows = ledger.filter((r) => r.balance_due > 0 || r.prev_balance > 0);
    if (search) {
    ...
    if (sortBy === "amount") {
      ...
      // oldest = most months overdue first
      rows = [...rows].sort(
        (a, b) =>
          safeNum(b.months_with_dues || 0) - safeNum(a.months_with_dues || 0),
      );
    }
    return rows;
          ...
          >
            ...
            <Box sx={{ display: "flex", gap: 0.5, ml: "auto" }}>
              ...
              <Chip
                ...
                sx={{
                  fontSize: "0.7rem",
                  bgcolor: sortBy === "oldest" ? "#FEE2E2" : undefined,
                  color: sortBy === "oldest" ? "#991B1B" : undefined,
                  fontWeight: sortBy === "oldest" ? 700 : 400,
                }}
              />
            </Box>
          </Box>
                      ...
                      >
                        ...
                        <TableCell>
                          <Chip
                            ...
                            sx={{
                              bgcolor:
                                r.status === "due" ? "#FEE2E2" : "#FEF3C7",
                              color: r.status === "due" ? "#991B1B" : "#92400E",
                              fontWeight: 700,
                              fontSize: "0.68rem",
                            }}
                          />
...
export default function PaymentsPage() {
  ...
  const [summary, setSummary] = useState({});
  const [ledger, setLedger] = useState([]); // PG-wide current month ledger
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(0);
  const [total, setTotal] = useState(0);
  const [paymentTab, setPaymentTab] = useState(0); // 0 = This Month, 1 = History
  ...
  const [selectedTenantInfo, setSelectedTenantInfo] = useState(null); // {tenant, ledgerRow}
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      // Tab filter: "This Month" restricts to current month
      if (paymentTab === 0) {
        params.month = currentMonthDate;
      }
  ...
  /* ── fetch ledger (current month, all tenants) for dashboard + outstanding table ── */
  ...
  const dashSummary = useMemo(() => {
    // Derive from ledger (source of truth) if available, else fall back to summary API
    if (ledger.length > 0) {
      return {
        ...
        currentDue: ledger
          ...
          .reduce((s, r) => s + safeNum(r.balance_due), 0),
        prevDueTotal: ledger.reduce((s, r) => s + safeNum(r.prev_balance), 0),
        countPaid: ledger.filter(
          (r) => r.status === "paid" || r.status === "overpaid",
        ).length,
        countDue: ledger.filter((r) => r.status === "due").length,
        countPartial: ledger.filter((r) => r.status === "partial").length,
      };
    }
    return {
      ...
      prevDueTotal: safeNum(summary.total_prev_dues || 0),
      countPaid: 0,
      countDue: 0,
      countPartial: 0,
    };
  ...
  /* ── find ledger row for selected tenant ── */
  ...
  const openAdd = (prefill = null) => {
    ...
    if (prefill) {
      // prefill from outstanding table click
  ...
  const balanceDueNum = safeNum(selRow?.balance_due); // current month gap
  const prevDueNum = safeNum(selRow?.prev_balance); // carry-forward
  ...
  const amountHelperText = () => {
    ...
    if (!form.amount) {
      if (prevDueNum > 0)
        return `Current due: ${fm(balanceDueNum)} + Prev: ${fm(prevDueNum)} = ${fm(totalOwed)} total`;
  ...
  const validate = () => {
    ...
    if (form.amount && safeNum(form.amount) > 999999) {
      errs.amount = "Amount seems too large — please verify";
      ok = false;
    }
    ...
    if (form.pg_tenant_id && form.amount && !editPayment && selRow) {
      if (amountNum > totalOwed) {
        errs.amount = `Cannot exceed ${fm(totalOwed)} (current due ${fm(balanceDueNum)} + prev ${fm(prevDueNum)})`;
        ok = false;
      }
    }
    setErrors(errs);
  ...
  /* ── row helpers ── */
  const isPartialRow = (p) =>
    p.month_status === "partial" ||
    p.is_partial ||
    safeNum(p.month_balance_due || p.balance_due) > 0;
  const paidPct = (p) => {
  ...
  // Tenants with outstanding dues — for dropdown (only show those with dues)
  const tenantsWithDues = ledger.filter(
    (r) => r.status !== "paid" && r.status !== "overpaid",
  );
    ...
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          },
          {
            ...
            sub: "Carry-forward from past months",
            color: "#DC2626",
            bg: "#FEE2E2",
          ...
          >
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <Typography sx={{ fontSize: "0.78rem", color: "#6B7280" }}>
                {fm(dashSummary.totalCollected)} collected of{" "}
                {fm(dashSummary.totalRent)}
              </Typography>
              <Chip
                label={`${collectedPct}%`}
                size="small"
                  ...
                  {payments.map((p) => {
                    ...
                    return (
                      <TableRow
                        ...
                        hover
                        onClick={() => {
                          if (canCreate) openEdit(p);
                        }}
                        sx={{
                          cursor: canCreate ? "pointer" : "default",
                          bgcolor: partial ? "#FFFBF5" : undefined,
                          "&:hover": {
                            bgcolor: partial ? "#FEF9ED" : "#F5F7FF",
                          },
                        }}
                      >
                        <TableCell>
                        ...
                        <TableCell>
                          ...
                          >
                            {p.month
                              ? new Date(p.month).toLocaleString("en-IN", {
                                  month: "short",
                                  year: "numeric",
                                })
                              : "—"}
                        ...
                        <TableCell>
                          ...
                          >
                            {fm(p.tenant_rent || p.monthly_rent)}
                          </Typography>
                        </TableCell>
                        <TableCell>
                          {partial ? (
                            <Chip
                              label={fm(p.month_balance_due || p.balance_due)}
                              size="small"
                              sx={{
                                bgcolor: "#FEF3C7",
                                color: "#92400E",
                        ...
                        <TableCell>
                          <StatusChip
                            status={
                              p.month_status ||
                              (partial ? "partial" : p.status || "settled")
                            }
                          />
                        </TableCell>
                        <TableCell>
      ...
      >
        ...
        <DialogContent sx={{ pt: 1 }}>
          ...
          {selRow && prevDueNum > 0 && (
            ...
            >
              ...
              <Typography sx={{ fontSize: "0.78rem", mt: 0.3 }}>
                To clear all dues, collect {fm(totalOwed)} (current due{" "}
                {fm(balanceDueNum)} + previous {fm(prevDueNum)}).
              </Typography>
              <Button
                size="small"
                variant="outlined"
          ...
          {/* Current month + prev balance info */}
          {selRow && prevDueNum === 0 && balanceDueNum > 0 && (
            <Alert
              severity="info"
              sx={{ mb: 2, borderRadius: 2, fontSize: "0.8rem" }}
            >
              Current month balance due: <strong>{fm(balanceDueNum)}</strong>
              {selRow.status === "partial" &&
                ` (partial — ${fm(safeNum(selRow.rent_amount) - balanceDueNum)} already paid)`}
            </Alert>
          )}

          <Grid container spacing={2} sx={{ mt: 0 }}>
            {/* Tenant — only show those with outstanding dues */}
            <Grid item xs={12}>
            ...
            {selRow && (
              <Grid item xs={12}>
                ...
                >
                  ...
                  {balanceDueNum > 0 && (
                    <Chip
                      label={`Current due ${fm(balanceDueNum)}`}
                      size="small"
                      clickable
                      variant="outlined"
                      onClick={() => {
                  ...
                  {balanceDueNum > 0 && (
                    <Chip
                      label={`Half ${fm(Math.round(safeNum(selRow.rent_amount) / 2))}`}
                      size="small"
                      clickable
                      variant="outlined"
                      onClick={() => {
                        setForm((f) => ({
                          ...f,
                          amount: Math.round(safeNum(selRow.rent_amount) / 2),
                        }));
                        setErrors((e) => ({ ...e, amount: "" }));
                      }}
                      sx={{ fontSize: "0.72rem" }}
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (selRow && !editPayment && val) {
                    if (safeNum(val) > totalOwed) {
                      setErrors((er) => ({
                        ...er,
                        amount: `Cannot exceed ${fm(totalOwed)}`,
                      }));
                    } else {
                ...
                inputProps={{
                  min: 1,
                  step: 1,
                  max: !editPayment && selRow ? totalOwed : undefined,
                }}
              />
            </Grid>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button
            ...
            variant="contained"
            disabled={saving || (allHavePaid && !editPayment)}
