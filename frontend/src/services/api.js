...
export const listingAPI = {
  getListings: (params) => {
    ...
    return fetch(
      `${import.meta.env.VITE_API_URL || "/api"}/listings?${qs}`,
    ).then((r) => r.json());
  },
  getListingById: (pgId) =>
    fetch(`${import.meta.env.VITE_API_URL || "/api"}/listings/${pgId}`).then(
      (r) => r.json(),
    ),
  getRecommendations: (pgId, limit = 4) =>
    fetch(
      `${import.meta.env.VITE_API_URL || "/api"}/listings/${pgId}/recommended?limit=${limit}`,
    ).then((r) => r.json()),
};
